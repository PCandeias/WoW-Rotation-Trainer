...
import { consumeShamanBuffStacks, ShamanAction } from '../shaman_action';
...
export class FlameShockAction extends ShamanAction {
  ...
  override preCastFailReason(): 'not_available' | undefined {
    return this.p.isBuffActive('voltaic_blaze') ? 'not_available' : undefined;
  }
  ...
  executeOnTarget(
    ...
    if (this.p.isBuffActive('voltaic_blaze')) {
      consumeShamanBuffStacks(this.p, 'voltaic_blaze', 1, newEvents);
    }
    ...
    const dotInstanceId = this.p.applyTargetDebuff?.('flame_shock', FLAME_SHOCK_DOT_DURATION_SECONDS, targetId, 1) ?? 0;
    const tickInterval = FLAME_SHOCK_BASE_TICK_INTERVAL_SECONDS / (1 + this.p.getHastePercent() / 100);
    const totalTicks = Math.max(1, Math.floor(FLAME_SHOCK_DOT_DURATION_SECONDS / tickInterval));
    const dotSnapshot = this.buildDotSnapshot(isComboStrike, targetId);

    for (let index = 0; index < totalTicks; index += 1) {
      result.newEvents.push({
        type: EventType.DOT_TICK,
        time: this.p.currentTime + tickInterval * (index + 1),
        spellId: this.name,
