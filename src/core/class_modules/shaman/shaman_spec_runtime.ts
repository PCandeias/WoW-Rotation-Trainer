...
import { SurgingBoltAction, TremorAction } from './actions/surging_totem';
...
export const shamanEnhancementRuntime: SpecRuntime = {
  ...
  processScheduledEvent(
    ...
    if (event.spellId === 'ascendance_doom_winds') {
      ...
      applyShamanBuffStacks(state, 'doom_winds', 1, buffEvents);
    ...
    if (event.spellId === 'alpha_wolf') {
      ...
      for (let targetId = 0; targetId < action.nTargets(); targetId += 1) {
        const impact = action.calculateDamage(rng, false);
    ...
    if (
      ...
      return {
        handled: true,
        damages: action.executeScheduledImpact(event.spellId, event.castContext, rng).damages,
      };
    }
